...
</style></head><body><div class="card"><div class="badge">!</div><div class="title">Can't open in file://</div><p class="desc">abOS Browser and web apps need a web server. Service Worker and Wisp can't run from <code>file://</code> (origin is <code>null</code>).</p><div class="actions"><button class="btn btn-primary" onclick='window.open(${JSON.stringify(pagesUrl)}, "_blank", "noopener")'>Open from abOS</button></div></div></body></html>`;
}
