  ...
  return `
    <div id="pane-accounts" class="settings-category-pane">
      ...
      <div class="accounts-cloud" id="accounts-cloud">
        <div class="accounts-cloud-head"><i class="fas fa-user-lock"></i> <span>abOS Account</span></div>
...
async function renderSyncPanel(bodyEl) {
  ...
  if (!loggedIn) {
    setHTML(
      bodyEl,
      `<p class="accounts-cloud-note"><i class="fas fa-lock"></i> Sign in to your abOS Account above to sync settings across devices. Cloud sync is off by default and everything works locally without it.</p>`
    );
    return;
  }
