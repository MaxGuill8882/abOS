...
export const FEATURE_DATA = {
  ...
  step6: {
    ...
    suggestedApps: [
      ...
      { id: "yukiOsGuide", title: "abOS Guide", icon: "fas fa-book-open" }
    ],
    transparencyLevels: [
...
export class SetupApp extends BaseApp {
  ...
  async open(options = {}) {
    ...
    const win = os.window.create(winId, "Set Up abOS", "85vw", "75vh", {
      icon: "fas fa-rocket",
      position: "center",
      skipHeader: true
    });
  ...
  buildUI() {
    const headerHtml = buildHeaderForStyle("Set Up abOS", "", os.window.getWindowControls(), resolveHeaderStyleId());
    return `
      ${headerHtml}
  ...
  buildStep6() {
    ...
    return `
      <div class="setup-step" data-step="7">
        ...
        <p style="font-size:13px;color:var(--text-secondary);margin:0 0 12px;">abOS stays fast with the keyboard. Start with these four, then explore the rest.</p>
        <div class="personalize-section">
...
Have fun!`;
    ...
    os.notify.send("Welcome to abOS", "", {
      type: "success",
      duration: 3000
    });
